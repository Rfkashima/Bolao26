...
const STORE_KEY = "bolao-copa-2026-picks-v2";

const state = {
  page: "inicio",
  picks: {},
  selectedPlayer: localStorage.getItem("bolao-player") || "",
  selectedPlayerCode: localStorage.getItem("bolao-player-code") || "",
...
function mergePicks(list) {
  list.forEach((pick) => {
    ...
    const g1 = Number(pick.g1 ?? pick.goals1);
    const g2 = Number(pick.g2 ?? pick.goals2);
  ...
  app.innerHTML = `
    ...
    <section class="grid two" style="margin-top:16px">
      ...
      <div class="card">
        ...
        ${rankingTable(ranking.slice(0, 11))}
      </div>
    </section>
  `;
}
...
function renderBetPage() {
  ...
  const backendText = DATA.settings.apiUrl
    ? "Digite o código individual do jogador para salvar na base oficial."
    : "Modo local: os palpites ficam salvos apenas neste navegador até conectar o Google Sheets.";
  ...
  app.innerHTML = `
    <section class="card">
      ...
      <div class="toolbar">
        ...
        <div class="field">
          <label>Código</label>
          <input id="playerCode" type="password" placeholder="Código do jogador" value="${escapeAttr(state.selectedPlayerCode)}">
        </div>
      ...
      <div class="notice">${backendText}</div>
  ...
  $("#playerCode").addEventListener("input", (e) => {
    state.selectedPlayerCode = e.target.value;
    localStorage.setItem("bolao-player-code", state.selectedPlayerCode);
  });
...
function saveRoundPicks(round) {
  ...
  const playerCode = ($("#playerCode")?.value || "").trim();
  state.selectedPlayerCode = playerCode;
  localStorage.setItem("bolao-player-code", playerCode);

  if (DATA.settings.apiUrl && !playerCode) {
    alert("Digite o código do jogador.");
    return;
  }
  ...
  for (const match of matches) {
    ...
    if (!Number.isInteger(goals1) || !Number.isInteger(goals2) || goals1 < 0 || goals2 < 0) {
      alert("Use apenas números inteiros e positivos nos placares.");
      return;
    }
...
function rankingTable(ranking) {
  return `<div class="table-wrap">
    <table>
      ...
      <tbody>
        ${ranking.map((row, index) => `<tr><td class="strong">${index + 1}º</td><td>${row.name}</td><td class="center strong">${row.points}</td><td class="center">${row.exacts}</td><td class="center">${row.results}</td></tr>`).join("")}
      </tbody>
    </table>
  </div>`;
}
    ...
    <section class="card">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${matches.map((m) => `<tr><td>${matchTitle(m)}</td>${DATA.players.map((p) => `<td class="center score-text">${shouldHide ? "Oculto" : formatPick(state.picks[p.id]?.[m.id])}</td>`).join("")}</tr>`).join("")}
          </tbody>
        </table>
      </div>
    </section>
    ...
    <section class="card">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${matches.map((m) => `<tr><td>${m.number}</td><td>${matchTitle(m)}</td><td>${formatDate(m.date)} às ${m.time}</td><td>${m.venue}</td><td class="center score-text">${formatResult(m)}</td></tr>`).join("")}
          </tbody>
        </table>
      </div>
    </section>
  ...
  return `<div class="card group-card">
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody>
          ${rows.map((r, index) => `<tr><td>${index + 1}</td><td>${r.team}</td><td class="center strong">${r.pts}</td><td class="center">${r.j}</td><td class="center">${r.sg}</td></tr>`).join("")}
        </tbody>
      </table>
    </div>
  </div>`;
  ...
  app.innerHTML = `
    <section class="card">
      ...
      <ul class="list">
        ...
        <li>O fechamento da rodada vale para todos os jogos daquela rodada.</li>
        <li>Critério de desempate inicial: maior número de placares exatos.</li>
      </ul>
    </section>
  `;
}
...
function renderPrizePage() {
  ...
  app.innerHTML = `
    ...
    <section class="card" style="margin-top:16px">
      ...
      <p>Essa área pode ser ajustada depois com os valores oficiais do seu bolão.</p>
    </section>
  `;
}
...
function roundDeadlineCard(round) {
  ...
  return `<div class="notice ${locked ? "danger" : ""}">
...
function matchTitle(match) {
  return `<div class="match-title">
    <span class="teams">${match.team1} x ${match.team2}</span>
    <span class="meta">${match.group} · Jogo ${match.number}</span>
  </div>`;
}
